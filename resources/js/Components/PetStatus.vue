...
const messageRoute = computed(() => petState.value["route"] || {});
...
const messageClass = computed(() => {
    return (petState.value["speak"] ?
        ...
        // link route
        (petState.value["route"] ? " underline cursor-pointer" : "");
});
...
if (petState.value["route"]) {
    clearPetState(props.pet._id, "route");
}
</script>

<template>
    <div :class="messageClass" class="mt-3 p-3 flex justify-center items-center h-[56px]"
         @click="useRoute(messageRoute)">
        <div>{{ message }}</div>
    </div>
</template>
